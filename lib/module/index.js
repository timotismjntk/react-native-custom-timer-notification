...
const CustomTimerNotification = NativeModules.CustomTimerNotification ? NativeModules.CustomTimerNotification : new Proxy({}, {
  get() {
    throw new Error(LINKING_ERROR);
  }
});
const CustomNotificationModule = NativeModules.CustomNotificationModule ? NativeModules.CustomNotificationModule : new Proxy({}, {
  get() {
    throw new Error(LINKING_ERROR);
  }
});
export function TimerNotification(a) {
  ...
  if (Platform.OS === 'android') return CustomTimerNotification.TimerNotification(data);
  return null;
}
export function RemoveTimer(a) {
...
export function CustomNotification(a, cb) {
  ...
  if (Platform.OS === 'android') return CustomNotificationModule.CustomNotification(data, cb);
  return null;
}
export function onEvent(listener) {
  DeviceEventEmitter.addListener('notificationClick', event => listener(event));
}
export const TYPES = {
  Image: 1,
  Text: 2,
